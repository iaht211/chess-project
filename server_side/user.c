...
int changePassword(char *username, char *oldPassword, char *newPassword)
{
    ...
    while (fgets(line, sizeof(line), file) != NULL)
    {
        ...
        if (storedUsername != NULL && storedPassword != NULL && storedElo != NULL && storedMatches != NULL)
        {
            // Kiểm tra xem tên người dùng có khớp không
            if (strcmp(username, storedUsername) == 0)
            {
                ...
                if (strcmp(oldPassword, storedPassword) == 0)
                {
                    ...
                    fprintf(tempFile, "%s %s %s %s\n", storedUsername, newPassword, storedElo, storedMatches);
                }
                else
                {
                    ...
                    fclose(file);
                    fclose(tempFile);
                    remove("temp_user.txt"); // Xóa tệp tạm
                    return 0;
                }
            }
            else
            {
                // Nếu không phải người dùng cần thay đổi mật khẩu, ghi lại thông tin ban đầu
                fprintf(tempFile, "%s %s %s %s\n", storedUsername, storedPassword, storedElo, storedMatches);
            }
        }
    }
